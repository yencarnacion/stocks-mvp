...
func TestCollectRubberBandBearishHistoryItemsIncludesEachNonEmptySnapshot(t *testing.T) {
	loc := mustLoadTestLoc(t)
	baseDay := time.Date(2026, 2, 19, 9, 30, 0, 0, loc)
	...
	mkSnap := func(minute int, rows []Candidate) TopSnapshot {
		...
		return TopSnapshot{
			GeneratedAt:                 ts,
			AsOfNY:                      ts.Format("2006-01-02 15:04:05 MST"),
			RubberBandBearishCandidates: rows,
			StrongestCandidates:         []Candidate{},
			WeakestCandidates:           []Candidate{},
			HardPassCandidates:          []Candidate{},
			BacksideCandidates:          []Candidate{},
	...
	items := collectRubberBandBearishHistoryItems(history, "2026-02-19", loc)
	if len(items) != 3 {
		t.Fatalf("expected 3 history items, got %d", len(items))
	}
...
func TestCollectEpisodicHistoryItemsIncludesEachNonEmptySnapshot(t *testing.T) {
	loc := mustLoadTestLoc(t)
	baseDay := time.Date(2026, 2, 19, 9, 30, 0, 0, loc)
	...
	history := []TopSnapshot{
		mkSnap(0, nil),
		mkSnap(1, []Candidate{{Rank: 1, Symbol: "ABC", Signal: "buy"}}),
		mkSnap(2, []Candidate{{Rank: 1, Symbol: "ABC", Signal: "sell"}}),
		mkSnap(3, nil),
		mkSnap(4, []Candidate{{Rank: 1, Symbol: "XYZ", Signal: "buy"}}),
	}

	items := collectEpisodicHistoryItems(history, "2026-02-19", loc)
	if len(items) != 3 {
		t.Fatalf("expected 3 history items, got %d", len(items))
	}

	if items[0].TimeLabel != "09:31" || items[0].Rows[0].Symbol != "ABC" || items[0].Rows[0].Signal != "buy" {
		t.Fatalf("unexpected first item: %+v", items[0])
	}
	if items[1].TimeLabel != "09:32" || items[1].Rows[0].Symbol != "ABC" || items[1].Rows[0].Signal != "sell" {
		t.Fatalf("unexpected second item: %+v", items[1])
	}
	if items[2].TimeLabel != "09:34" || items[2].Rows[0].Symbol != "XYZ" {
		t.Fatalf("unexpected third item: %+v", items[2])
